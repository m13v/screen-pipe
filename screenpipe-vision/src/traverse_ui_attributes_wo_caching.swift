...
func printAllAttributeValues(_ startElement: AXUIElement) -> [String] {
    ...
    // print("\(round(totalTime * 1000)) ms total execution time, \(outputLength) characters total output length")
...
func printAllAttributeValuesForCurrentApp() -> [String] {
    ...
    let uniqueValues = printAllAttributeValues(axApp)
    let totalTime = CFAbsoluteTimeGetCurrent() - startTime
    let outputLength = uniqueValues.joined().count
    
    print("time: \(Int(round(totalTime * 1000)))ms, chars: \(outputLength), app: \(app.localizedName?.lowercased() ?? "unknown"), window: \(windowName.lowercased())")
